export const styles = {
  layout: {
    title: `
      text-center text-4xl md:text-5xl font-serif font-extrabold
      ...
      mb-6 mt-2
    `,
    main: "max-w-2xl mx-auto p-4 font-mono select-none",
    // Updated Arena to match the Dark Stone theme (removed gray-100)
    arena:
      "border-4 border-stone-600 rounded-lg p-6 bg-stone-800 relative h-96 mb-4 flex flex-row justify-between shadow-xl",
    bottomGrid: "grid grid-cols-2 gap-4 h-40",
  },
  actionBar: {
    container: `
        grid grid-cols-2 gap-3 p-3 rounded-xl relative overflow-hidden
        bg-stone-900 border-4 border-stone-600
  ...
  winner: {
    base: `
        ...
        font-serif font-bold text-lg tracking-[0.2em] uppercase
